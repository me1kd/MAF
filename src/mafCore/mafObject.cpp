...
mafObject::mafObject(const QString code_location) : mafObjectBase(code_location), m_IconType(""), m_LockStatus(mafObjectLockNone), m_ProgressStatus(0),  m_TagList(NULL), m_Dictionary(NULL) {
...
void mafObject::setIconType(QString icon_type) {
    ...
    Q_FOREACH(item, *memento->mementoPropertyList()) {
        ...
        if(name != "objectHash" || deep_memento) {
            if( name.compare("TagList") == 0 ) {
                ...
                int index = this->metaObject()->indexOfProperty(name.toLatin1());
                if (index == -1) {
                    qWarning() << "Trying to restore a property not found";
                } else {
                    if(this->metaObject()->property(index).isWritable()) {
                        this->setProperty(name.toLatin1(), item.m_Value);
                    }
                }
            }
        }
